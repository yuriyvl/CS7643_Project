...
from fastmri.pl_modules import FastMriDataModule, AttnUnetModule
...
def build_args():
    ...
    default_root_dir = fetch_dir("log_path", path_config) / "attnunet" / "attnunet_demo"
    ...
    # module config
    parser = AttnUnetModule.add_model_specific_args(parser)
    parser.set_defaults(
        in_chans=1,  # number of input channels to Attn-U-Net
        out_chans=1,  # number of output chanenls to Attn-U-Net
        chans=32,  # number of top-level Attn-U-Net channels
        num_pool_layers=4,  # number of Attn-U-Net pooling layers
    ...
    if args.recon == True:
        checkpoint_dir = pathlib.Path("../../results/attunet")
    else:
