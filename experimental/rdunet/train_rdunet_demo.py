...
from fastmri.pl_modules import FastMriDataModule, ResidualDenseUNetModule
...
def build_args():
    ...
    default_root_dir = fetch_dir("log_path", path_config) / "rdunet" / "rdunet_demo"
    ...
    # module config
    parser = ResidualDenseUNetModule.add_model_specific_args(parser)
    parser.set_defaults(
        in_chans=1,  # number of input channels to Dense U-Net
        out_chans=1,  # number of output chanenls to Dense U-Net
        chans=32,  # number of top-level Dense U-Net channels
        num_pool_layers=2,  # number of Dense U-Net pooling layers
        num_depth_blocks=1,
        growth_rate=16,
    ...
    if args.recon == True:
        checkpoint_dir = pathlib.Path("../../results/residualdenseunet")
    else:
