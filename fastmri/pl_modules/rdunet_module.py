...
import torch
from fastmri.models import ResidualDenseUNet
...
class ResidualDenseUNetModule(MriModule):
    """
    ResidualDenseUNet training module.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rdunet = ResidualDenseUNet(
            in_chans=self.in_chans,
            out_chans=self.out_chans,
            chans=self.chans,
            num_pool_layers=self.num_pool_layers,
            num_depth_blocks=self.num_depth_blocks,
            growth_rate=self.growth_rate,
            num_layers_per_block=self.num_layers_per_block
        )

    def forward(self, image):
        return self.rdunet(image.unsqueeze(1)).squeeze(1)

    def training_step(self, batch, batch_idx):
        image, target, _, _, _, _, _ = batch
        output = self(image)
        # make_dot(output, params=dict(list(self.dunet.named_parameters()))).render("dunet_torchviz.png", format="png")
        # sys.exit()
